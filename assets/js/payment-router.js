...
(function () {
  ...
  function determineRoute(jobData) {
    ...
    // 2. Invoice Gate (Download/View Step)
    const hasInvoice = !!clientStatus.invoice;
    if (!hasInvoice) {
      return { route: 'invoice', paymentNumber: 1, reason: 'Invoice not viewed' };
    }
    ...
    // 4. Balance Gate (Download/View Step) - Only if 2 payments
    const hasBalance = !!clientStatus.balance;
    if (totalPayments > 1 && !hasBalance) {
      return { route: 'invoice', paymentNumber: 2, reason: 'Balance Invoice not viewed' };
    }
