...
* These test that followed the fix, all detailed above, show that there are still events that trigger a separate API call to `/api/track-event` when the user exits the page.
  ...
  - If the "skipped" or "already processed" are part of the same event as when it _isn't_ skipped or _is_ processed, then ideally we should find a new way to handle these; now that things are working properly, we don't need to know if something was skipped or already processed
