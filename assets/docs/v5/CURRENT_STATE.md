...
4.  **Optimistic UI**: When an action is taken (Sign/Pay), the local state updates _immediately_ to show the next step, while the backend processes the event in the background.
...
_Audit of `assets/docs/v5/PROJECT_OVERVIEW.md` vs Current Reality_

- The `PROJECT_OVERVIEW.md` is largely accurate in _intent_, but the _implementation details_ have changed:
  ...
  3. **Frontend Files**: References to `job.html` or `index.html` as templates are slightly misleading. Vite compiles everything into a single JS bundle that _injects_ into these HTML files
