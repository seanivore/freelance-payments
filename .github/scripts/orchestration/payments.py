...
def execute_8_step_sync(jobs_dir: str, manifest_path: str, trigger_category: str = 'payment') -> dict:
    ...
    print(f"[TRIGGER={trigger_category}] Step 3: Found {len(json_job_ids)} JSON file(s) in directory ({json_active_count} active={True}, {json_inactive_count} active={False})", file=sys.stderr)
    ...
    print(f"[TRIGGER={trigger_category}] Step 3: Found {stripe_active_count} active product(s) and {stripe_archived_count} archived product(s) in Stripe catalog", file=sys.stderr)
    ...
    print(f"[TRIGGER={trigger_category}] Step 7: Found 0 JSON file(s), active=N/A and Found {archived_count} catalog product(s), active=False", file=sys.stderr)
    
    if archived_count > 0:
        print(f"[TRIGGER={trigger_category}] Step 7: RESULT - Ignoring {archived_count} orphaned archived product(s)", file=sys.stderr)
    else:
