...
def main():
    ...
    try:
        ...
        print(f"[TRIGGER=admin-push] Step 1: Comparing JSONs to catalog", file=sys.stderr)
        sync_stats = execute_8_step_sync(jobs_dir, manifest_path, 'admin-push')
        results['steps_run'].append('sync_catalog')
        print(f"[TRIGGER=admin-push] Step 1: Sync stats: {json.dumps(sync_stats, indent=2)}", file=sys.stderr)
        
        # Step 2: Generate PDFs for new jobs
        new_job_ids = []
        if sync_stats.get('products_created', 0) > 0:
            ...
            if new_job_ids:
                print(f"[TRIGGER=admin-push] Step 2: Generating PDFs for {len(new_job_ids)} new job(s)", file=sys.stderr)
                ...
                print(f"[TRIGGER=admin-push] Step 2: PDF generation completed ({pdf_stats.get('contracts_generated', 0)} contracts, {pdf_stats.get('invoices_generated', 0)} invoices)", file=sys.stderr)
            else:
                print(f"[TRIGGER=admin-push] Step 2: Skipping PDF generation (no new jobs detected)", file=sys.stderr)
        else:
            print(f"[TRIGGER=admin-push] Step 2: Skipping PDF generation (no new products)", file=sys.stderr)
        ...
        try:
            ...
            print(f"[TRIGGER=admin-push] Step 10: Manifest generated", file=sys.stderr)
        except Exception as e:
        ...
        # Step 12: Deploy
        print(f"[TRIGGER=admin-push] Step 12: Deploying", file=sys.stderr)
        commit_message = "🤖 Auto-update: Stripe catalog sync and manifest update\n\nCo-Authored-By: GitHub Actions <action@github.com>"
        if git_commit_and_push(commit_message):
            results['committed'] = True
            results['pushed'] = True
            print(f"[TRIGGER=admin-push] Step 12: Changes committed and pushed", file=sys.stderr)
        else:
            print(f"[TRIGGER=admin-push] Step 12: No changes detected - skipping commit", file=sys.stderr)
